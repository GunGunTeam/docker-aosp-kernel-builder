...
RUN set -x \
	...
	&& apt-get install --no-install-recommends --no-install-suggests -y git-core gnupg flex bison gperf build-essential zip curl zlib1g-dev gcc-multilib g++-multilib libc6-dev-i386 lib32ncurses5-dev x11proto-core-dev libx11-dev lib32z-dev libgl1-mesa-dev libxml2-utils xsltproc bc liblz4-tool unzip libssl-dev device-tree-compiler \
	\
	&& mkdir -p /opt/ndk \
